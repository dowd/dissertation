...
ggplot(os_results, aes(fill=(Points.Distance<20),  x=Type)) + geom_bar(position="fill")+scale_fill_manual(values=cbPalette)+coord_flip()+ggtitle("under 20m")+theme(legend.position="bottom",legend.title = element_blank())+labs(y="Percentage")
...
ggplot(gsv_results, aes(fill=(Points.Distance<20),  x=Type)) + geom_bar(position="fill")+scale_fill_manual(values=cbPalette)+coord_flip()+theme(legend.position="bottom",legend.title = element_blank())+labs(y="Percentage")
```
...
kable(results,caption="Table x - showing attribute counts")
...
kable(t(results_relevant),caption="Table x - showing Version all results")
kable(t(results),caption="Table x - showing Version relevant results")
...
kable(sauce2,caption="Table x - showing whether they have source tag")
kable(sauce3,caption="Table x - showing number of elements in OSM")
...
kable(aware1,caption="Table x - showing answer to aware questionns")
...
for (type in types) {
  num=length(grep(type, questionnaire$Contributed))
  print(paste(num,type))
}
